...
do
	...
	if gethiddenproperty then
		...
		elseif EXECUTOR_NAME ~= "Synapse Z" then
...
do
	...
	local function FetchAPI()
		...
		local ok, err = pcall(function()
			...
			local function try_add_hash(url)
				local o, r = pcall(game.HttpGet, game, url, true)
				if o and r and r ~= "" then
					local hash = string.match(r, "(version%-[^%s\r\n]+)")
					if hash then matching_versions[hash] = true return true end
				end
			end
			try_add_hash("https://setup.rbxcdn.com/versionQTStudio")
			try_add_hash("https://setup.rbxcdn.com/ClientVersionUpload")

			if not next(matching_versions) then
				local is_matched
				local ok_dh, raw_dh = pcall(game.HttpGet, game, "https://setup.rbxcdn.com/DeployHistory.txt", true)
				if ok_dh and raw_dh then
					for _, line in string.split(raw_dh, "\n") do
						local fv = string.match(line, "file version: ([%d, ]+)")
						if fv then
							if string.split(fv, ", ")[2] == CLIENT_VERSION_str then
								is_matched = true
								local vh = string.match(line, "(version%-[^%s]+)")
								if vh then matching_versions[vh] = true end
							elseif is_matched then break end
						end
					end
				end
			end
		...
		if not ok or not API_Dump then
			local reason = not ok and tostring(err)
				or not API_Dump and "Version " .. version() .. " not found in DeployHistory or all CDN fetches failed"
				or "unknown"
			warn("[DEBUG] Failed to get " .. version() .. " API Dump, trying latest.. Reason: " .. reason)
			...
			if fb_ok and fb_result then
				...
				warn("[DEBUG] GitHub fallback also failed. Cannot load API Dump.")
				return nil
			end
		end
		...
		for _, API_Class in service.HttpService:JSONDecode(API_Dump) do
			...
			if ClassTags then
				Class.Tags = ArrayToDictionary(ClassTags, nil, nil, "string")
			end
						...
						then
							...
							if NotScriptableFixClass then
								...
								if NotScriptableFix then
									...
									if type(NotScriptableFix) == "string" then
										Property.DefaultKey = NotScriptableFix
									end
								end
...
local function synsaveinstance(CustomOptions, CustomOptions2)
	...
	do
		...
		if IsModel then
			...
			local gameName
			do
				local spaceIdx = string.find(tostring(PlaceName), " ", 1, true)
				gameName = spaceIdx and string.sub(tostring(PlaceName), spaceIdx + 1) or tostring(PlaceName)
			end
			local baseName = FilePath or sanitizeFileName("Place_" .. gameName)
			placename = baseName .. ".rbxlx"
			if isfile(placename) then
				local n = 1
				repeat
					placename = baseName .. "(" .. n .. ").rbxlx"
					n += 1
				until not isfile(placename)
			end
		end
	...
	local function ReturnItem(className, instance)
		...
		if className == "Terrain" and realcheck then
			...
			if ok_sg and ok_pg and smoothgrid_raw and physicsgrid_raw then
				...
				if write_ok then
					local read_ok, physicsgrid, smoothgrid = pcall(function()
						return to_base64(readfile("PhysicsGrid.txt")),
							to_base64(readfile("SmoothGrid.txt"))
					end)

					if read_ok and physicsgrid and smoothgrid then
						...
						warn("[SaveTerrain] Échec de la lecture des fichiers SmoothGrid/PhysicsGrid après écriture.")
					end
				else
					warn("[SaveTerrain] Échec de l'écriture des fichiers SmoothGrid/PhysicsGrid (permissions ?).")
				end
			else
				warn("[SaveTerrain] SmoothGrid ou PhysicsGrid retourne nil — données terrain ignorées.")
			end
		end
		return itemstring
	end
			...
			if InstanceOverride and InstanceOverride.__SaveSpecific then
				...
				if not (IgnorePropertiesOfNotScriptsOnScriptsMode and not isLuaSourceContainer(instance)) then
					...
					for _, Property in proplist do
						...
						then
							local defaultKey = Property.DefaultKey or PropertyName
